  ...
  caelestia-scripts = pkgs.stdenv.mkDerivation rec {
    ...
    patchPhase = ''
      ...
      find . -name "*.py" -type f | while read -r file; do
        ...
        sed -i 's|apply_discord(gen_scss(colours))|#apply_discord(gen_scss(colours))|g' "$file"
      done
    '';
    ...
    installPhase = ''
      ...
      if [ ! -d "\$DATA_HOME/schemes" ] && [ -d "$out/share/caelestia-scripts/src/caelestia/data/schemes" ]; then
        cp -r "$out/share/caelestia-scripts/src/caelestia/data/schemes" "\$DATA_HOME/"
      fi
      if [ ! -d "\$DATA_HOME/templates" ] && [ -d "$out/share/caelestia-scripts/src/caelestia/data/templates" ]; then
        cp -r "$out/share/caelestia-scripts/src/caelestia/data/templates" "\$DATA_HOME/"
      fi
      if [ ! -f "\$DATA_HOME/config.json" ] && [ -f "$out/share/caelestia-scripts/data/config.json" ]; then
        cp "$out/share/caelestia-scripts/src/caelestia/data/config.json" "\$DATA_HOME/"
      fi
      if [ ! -f "\$DATA_HOME/emojis.txt" ] && [ -f "$out/share/caelestia-scripts/data/emojis.txt" ]; then
        cp "$out/share/caelestia-scripts/src/caelestia/data/emojis.txt" "\$DATA_HOME/"
      fi
      EOF
      chmod +x $out/bin/caelestia-setup

      # Create wrapper for main script with all required tools in PATH
      makeWrapper ${pkgs.zsh}/bin/zsh $out/bin/caelestia \
